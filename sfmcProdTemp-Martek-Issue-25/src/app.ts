...
app.use(
  session({
    ...
    cookie: { secure: process.env.RUNNING_IN_HEROKU ? true : false, 
...
app.post("/fetchintelliseedlist", auth.isSparkpostVerified, function (req, res) {
  appDemoRoutes.fetchintelliseedlist(req, res);
  //intelliseedSession[req.body.accountId] = req.session;
});

//
app.get("/getintelliseedsession", auth.isSparkpostVerified, function (req, res) {
...
app.get("/getsessionavailabledomains", auth.isSparkpostVerified, function (req, res) {
