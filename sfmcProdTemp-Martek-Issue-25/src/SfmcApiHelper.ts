...
export default class SfmcApiHelper {
   ...
   */
  public CheckAutomationStudio(accessToken: string, jobStatId: string, bounceDEId: string, endpoint: string, member_id: string) {
    let self = this;
    ...
    let existAutomatedData = '<?xml version="1.0" encoding="UTF-8"?>'
      + '<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:a="http://schemas.xmlsoap.org/ws/2004/08/addressing" xmlns:u="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd">'
      + '    <s:Header>'
      + '        <a:Action s:mustUnderstand="1">Create</a:Action>'
      + `        <a:To s:mustUnderstand="1">${endpoint.replace('.rest.', '.soap.')}Service.asmx</a:To>`
    ...
    return new Promise<any>((resolve, reject) => {
      axios(existConfig)
        .then((response: any) => {
          ...
          parser.parseString(rawdata, async function (err: string, result: any) {

            if (JSON.stringify(result['soap:Envelope']['soap:Body'][0]['RetrieveResponseMsg'][0]['Results'][0]).includes("No rows were found")) {
              self.creatingAutomationStudio(accessToken, jobStatId, bounceDEId, endpoint, member_id)
                .then((response: any) => {
                  console.log('creatingAutomation Studio response --> ' + JSON.stringify(response));
                })
                .catch((error: any) => {
                  console.log('creatingAutomation Studio error --> ' + JSON.stringify(error));
                });
            } else {
              console.log('Existing automation Name: ' + JSON.stringify(result['soap:Envelope']['soap:Body'][0]['RetrieveResponseMsg'][0]['Results'][0]['Name']));
              resolve({
                statusText: JSON.stringify(result['soap:Envelope']['soap:Body'][0]['RetrieveResponseMsg'][0]['Results'][0]['Name']),
              });
            }
          })
        })
  ...
  async creatingAutomationStudio(accessToken: string, jobStatId: string, bounceDEId: string, endpoint: string, member_id: string) {

    let queryFolderResponse: any = await this.getQueryFolderId(accessToken, endpoint);
    console.log('CreateAutomationStudio queryFolderResponse' + JSON.stringify(queryFolderResponse));
    ...
    let emailResponse: any = await this.CreateQueryActivity(accessToken, endpoint, emailStats);
    console.log('CreateAutomationStudio emailResponse' + JSON.stringify(emailResponse));
    ...
    let openResponse: any = await this.CreateQueryActivity(accessToken, endpoint, openStats);
    ...
    let clicksResponse: any = await this.CreateQueryActivity(accessToken, endpoint, clicksStats);
    ...
    let unsubResponse: any = await this.CreateQueryActivity(accessToken, endpoint, unsubStats);
    ...
    let bounceResponse: any = await this.CreateQueryActivity(accessToken, endpoint, bounceStats);
    ...
    let bounceDetailsResponse: any = await this.CreateQueryActivity(accessToken, endpoint, bounceDetails);

    //queryDefinitionId//name//key
    let automationData = '<?xml version="1.0" encoding="UTF-8"?>'
      + '<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:a="http://schemas.xmlsoap.org/ws/2004/08/addressing" xmlns:u="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd">'
      + '    <s:Header>'
      + '        <a:Action s:mustUnderstand="1">Create</a:Action>'
      + `        <a:To s:mustUnderstand="1">${endpoint.replace('.rest.', '.soap.')}Service.asmx</a:To>`
      ...
      + '              <AutomationTasks>'
      + '                <AutomationTask>'
      + '                    <Name>' + emailResponse.name + '</Name>'
      + '                    <Activities>'
      + '                       <Activity>'
      + '                          <ObjectID>' + emailResponse.queryDefinitionId + '</ObjectID>'
      + '                          <ActivityObject xsi:type="QueryDefinition">'
      + '                              <ObjectID>' + emailResponse.queryDefinitionId + '</ObjectID>'
      + '                              <CustomerKey>' + emailResponse.key + '</CustomerKey>'
      + '                              <Name>' + emailResponse.name + '</Name>'
      + '                          </ActivityObject>'
      + '                      </Activity>'
      + '                  </Activities>'
      + '                </AutomationTask>'
      + '                <AutomationTask>'
      + '                    <Name>' + openResponse.name + '</Name>'
      + '                    <Activities>'
      + '                       <Activity>'
      + '                          <ObjectID>' + openResponse.queryDefinitionId + '</ObjectID>'
      + '                          <ActivityObject xsi:type="QueryDefinition">'
      + '                              <ObjectID>' + openResponse.queryDefinitionId + '</ObjectID>'
      + '                              <CustomerKey>' + openResponse.key + '</CustomerKey>'
      + '                              <Name>' + openResponse.name + '</Name>'
      + '                          </ActivityObject>'
      + '                      </Activity>'
      + '                  </Activities>'
      + '                </AutomationTask>'
      + '                <AutomationTask>'
      + '                    <Name>' + clicksResponse.name + '</Name>'
      + '                    <Activities>'
      + '                       <Activity>'
      + '                          <ObjectID>' + clicksResponse.queryDefinitionId + '</ObjectID>'
      + '                          <ActivityObject xsi:type="QueryDefinition">'
      + '                              <ObjectID>' + clicksResponse.queryDefinitionId + '</ObjectID>'
      + '                              <CustomerKey>' + clicksResponse.key + '</CustomerKey>'
      + '                              <Name>' + clicksResponse.name + '</Name>'
      + '                          </ActivityObject>'
      + '                      </Activity>'
      + '                  </Activities>'
      + '                </AutomationTask>'
      + '                <AutomationTask>'
      + '                    <Name>' + unsubResponse.name + '</Name>'
      + '                    <Activities>'
      + '                       <Activity>'
      + '                          <ObjectID>' + unsubResponse.queryDefinitionId + '</ObjectID>'
      + '                          <ActivityObject xsi:type="QueryDefinition">'
      + '                              <ObjectID>' + unsubResponse.queryDefinitionId + '</ObjectID>'
      + '                              <CustomerKey>' + unsubResponse.key + '</CustomerKey>'
      + '                              <Name>' + unsubResponse.name + '</Name>'
      + '                          </ActivityObject>'
      + '                      </Activity>'
      + '                  </Activities>'
      + '                </AutomationTask>'
      + '                <AutomationTask>'
      + '                    <Name>' + bounceResponse.name + '</Name>'
      + '                    <Activities>'
      + '                       <Activity>'
      + '                          <ObjectID>' + bounceResponse.queryDefinitionId + '</ObjectID>'
      + '                          <ActivityObject xsi:type="QueryDefinition">'
      + '                              <ObjectID>' + bounceResponse.queryDefinitionId + '</ObjectID>'
      + '                              <CustomerKey>' + bounceResponse.key + '</CustomerKey>'
      + '                              <Name>' + bounceResponse.name + '</Name>'
      + '                          </ActivityObject>'
      + '                      </Activity>'
      + '                  </Activities>'
      + '                </AutomationTask>'
      + '                <AutomationTask>'
      + '                    <Name>' + bounceDetailsResponse.name + '</Name>'
      + '                    <Activities>'
      + '                       <Activity>'
      + '                          <ObjectID>' + bounceDetailsResponse.queryDefinitionId + '</ObjectID>'
      + '                          <ActivityObject xsi:type="QueryDefinition">'
      + '                              <ObjectID>' + bounceDetailsResponse.queryDefinitionId + '</ObjectID>'
      + '                              <CustomerKey>' + bounceDetailsResponse.key + '</CustomerKey>'
      + '                              <Name>' + bounceDetailsResponse.name + '</Name>'
      + '                          </ActivityObject>'
      + '                      </Activity>'
      + '                  </Activities>'
      ...
      axios(config)
        .then((response: any) => {
          ...
          parser.parseString(rawdata, function (err: string, result: any) {
            ...
            let status_Code = result['soap:Envelope']['soap:Body'][0]['CreateResponse'][0]['Results'][0]['StatusCode'];
            if (status_Code == 'OK') {
              ...
              let status_Msg = result['soap:Envelope']['soap:Body'][0]['CreateResponse'][0]['Results'][0]['StatusMessage'];
              console.log('Creating automation status_Code: ' + status_Code + ' Status Msg :' + status_Msg);
              reject({
                statusText: status_Code,
              });
  ...
  public CreateQueryActivity(accessToken: string, endpoint: string, queryPayload: any) {
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      axios(config)
        .then((response: any) => {
          console.log(`creating ${queryPayload.name} Query Activity` + response.data);
      ...
      .then((response) => {
        ...
        return new Promise<any>((resolve, reject) => {
          ...
          axios({
            ...
            .then((response: any) => {
              var countablejourney=[];
              for(var i=req.query.strtpoint;i<=req.query.endpoint;i++){
                countablejourney.push(response.data.items[i]);
              }
            console.log('countablejourney:'+countablejourney);
              let sendresponse = {
                refreshToken: refreshTokenbody,
                journeys: response.data,
                countablejourneys:countablejourney,
                journeycount:response.data.count
              };
              res.status(200).send(sendresponse);
            })
            .catch((error: any) => {
              ...
              parser.parseString(
                ...
                (
                  ...
                  if (sendJobStats != undefined) {
                    let sendresponse = {
                      jobStatsId: JSON.stringify(result["soap:Envelope"]["soap:Body"][0]["RetrieveResponseMsg"][0]["Results"][0]["ObjectID"]).replace(/[^\w\s]/gi, ''),
                      refreshToken: refreshTokenbody,
            ...
            .then((response: any) => {
              ...
              parser.parseString(
                ...
                (
                  ...
                  console.log(' creating job stats : ' + JSON.stringify(result["soap:Envelope"]["soap:Body"][0]["CreateResponse"][0]["Results"][0]["StatusMessage"]).replace(/[^\w\s]/gi, ''));
                  let sendresponse = {};
                  sendresponse = {
                    jobStatsId: JSON.stringify(result["soap:Envelope"]["soap:Body"][0]["CreateResponse"][0]["Results"][0]["NewObjectID"]).replace(/[^\w\s]/gi, ''),
                    refreshToken: refreshTokenbody,
                    statusText:
              ...
              parser.parseString(
                ...
                (
                  ...
                  if (sendJobStats != undefined) {
                    ...
                    this.CheckAutomationStudio(accessToken, req.body.jobStatId, JSON.stringify(result["soap:Envelope"]["soap:Body"][0]["RetrieveResponseMsg"][0]["Results"][0]["ObjectID"]).replace(/[^\w\s]/gi, ''), req.body.soapInstance.replace('.soap.', '.rest.'), req.body.memberid);
                    //  res.status(200).send("Send Job stats already created");
                    this.CheckAutomationStudio(accessToken, req.body.jobStatId, JSON.stringify(result["soap:Envelope"]["soap:Body"][0]["RetrieveResponseMsg"][0]["Results"][0]["ObjectID"]).replace(/[^\w\s]/gi, ''), req.body.soapInstance.replace('.soap.', '.rest.'), req.body.memberid);
                    res.status(200).send(sendresponse);
                    /*   res
            ...
            .then((response: any) => {
              ...
              parser.parseString(
                ...
                (
                  ...
                  console.log(' creating bounce stats : ' + JSON.stringify(result["soap:Envelope"]["soap:Body"][0]["CreateResponse"][0]["Results"][0]["StatusMessage"]).replace(/[^\w\s]/gi, ''));
                  let sendresponse = {};
                  sendresponse = {
                    refreshToken: refreshTokenbody,
                    statusText:
                  ...
                  this.CheckAutomationStudio(accessToken, req.body.jobStatId, JSON.stringify(result["soap:Envelope"]["soap:Body"][0]["CreateResponse"][0]["Results"][0]["NewObjectID"]).replace(/[^\w\s]/gi, ''), req.body.soapInstance.replace('.soap.', '.rest.'), req.body.memberid);
                  res.status(200).send(sendresponse);
                  /* res
                .status(200)
                .send(
